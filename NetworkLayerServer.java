...
public class NetworkLayerServer {
    ...
    public static synchronized void DVR(int startingRouterId) throws InterruptedException {
        /**
         * pseudocode
         */
        ...
        */
        ...
        System.out.println("Entering DVR ");
        /*if (stateChanger != null){
            synchronized (stateChanger) {
                stateChanger.wait();
            }
        ...
        boolean[] routingtableChange = new boolean[routers.size()];
        while (convergence)
        {
            System.out.println("MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM");
            Arrays.fill(routingtableChange,false);
            Router startingRouter = routerMap.get(startingRouterId);
            for (int i = 0; i < startingRouter.getNeighborRouterIDs().size(); i++) {
                Router neighbour = routerMap.get(startingRouter.getNeighborRouterIDs().get(i));
                boolean c = startingRouter.sfupdateRoutingTable(neighbour);
                if (c == true) {
                    routingtableChange[routers.indexOf(startingRouter)] = true ;
                }
            }
            ...
            for (int i = 0; i < routers.size() ; i++) {
                ...
                {
                    for (int j = 0; j < routers.get(i).getNeighborRouterIDs().size(); j++) {
                        Router neighbour = routerMap.get(routers.get(i).getNeighborRouterIDs().get(j));
                        boolean c = routers.get(i).sfupdateRoutingTable(neighbour);
                        if (c == true)
                        {
                            routingtableChange[i] = true ;
                        }
                    }
                }
            }
            int flag = 0;
            for (int i = 0; i < routingtableChange.length; i++) {
                if (routingtableChange[i] == true) {
                    flag = 1 ;
                    convergence = true ;
                    break;
                }
    ...
    public static synchronized void simpleDVR(int startingRouterId) throws InterruptedException {
        ...
        while (convergence)
        {
            ...
            for (int i = 0; i < startingRouter.getNeighborRouterIDs().size(); i++) {
                Router neighbour = routerMap.get(startingRouter.getNeighborRouterIDs().get(i));
                boolean c = startingRouter.updateRoutingTable(neighbour);
                if (c == true) {
                    routingtableChange[routers.indexOf(startingRouter)] = true ;
                }
            }
            ...
            for (int i = 0; i < routers.size() ; i++) {
                if(routers.get(i).getRouterId() != startingRouterId)
                {
                    for (int j = 0; j < routers.get(i).getNeighborRouterIDs().size(); j++) {
                        Router neighbour = routerMap.get(routers.get(i).getNeighborRouterIDs().get(j));
                        boolean c = routers.get(i).updateRoutingTable(neighbour);
                        if (c == true)
                        {
                            routingtableChange[i] = true ;
                        }
